...
then
	if [ "$CHECKDEVICE" = "eon" ]
	then
		pkglist=(raspi-gpio python3-rpi.gpio python3-smbus i2c-tools smartmontools)	
	else
		pkglist=(raspi-gpio python3-rpi.gpio python3-smbus i2c-tools)	
	fi
elif [ "$CHECKPLATFORM" = "Ubuntu" ]
then
	# Todo handle lgpio
	# Ubuntu has serial and i2c enabled
	if [ "$CHECKDEVICE" = "eon" ]
	then
		pkglist=(python3-rpi.gpio python3-smbus i2c-tools smartmontools)
	else
		pkglist=(python3-rpi.gpio python3-smbus i2c-tools)
	fi
elif [ "$CHECKPLATFORM" = "Manjaro" ]
then
	if [ "$CHECKDEVICE" = "eon" ]
	then
...
then
	for curpkg in ${pacman_pkglist[@]}; do
		sudo pacman -S --noconfirm $curpkg
	...
	python3 -m pip install smbus rpi-gpio
